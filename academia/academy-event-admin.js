(() => {
  ...
  let timer=null,opsWrapped=false;
  ...
  function remount(kind){if(section()!==kind)return;[40,180,420,900].forEach(delay=>setTimeout(()=>{if(kind==='support'){guardOperations();cleanupSupportLegacy();}window.ACADEMIA_YAMILET_ADMIN?.render?.();setTimeout(()=>{if(kind==='support'){cleanupSupportLegacy();window.ACADEMIA_YAMILET_SUPPORT_ADMIN_V86?.render?.();}else window.ACADEMIA_YAMILET_AGENDA_ADMIN_V85?.render?.();},35);},delay));}
  function load(kind){
