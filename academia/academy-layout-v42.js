(() => {
  ...
  function mountFocus(){
    ...
    if(!wrapper){wrapper=document.createElement('section');wrapper.className='home-v42-focus';continuePanel.before(wrapper)}
    wrapper.append(continuePanel,progress);
    mounted=true;
    document.body.dataset.academyLayout='v42';
    return true;
  }
  function boot(){let attempts=0;if(mountFocus())return;const timer=setInterval(()=>{attempts+=1;if(mountFocus()||attempts>=80)clearInterval(timer)},100)}
