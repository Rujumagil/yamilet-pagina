(function(){
  ...
  function upgradeVisualAssets(){
    // Siempre preferimos los PNG oficiales HD cuando existen.
    document.querySelectorAll('img[src$=".webp"]').forEach(img => {
      const current = img.getAttribute('src');
      if(current) probeAndSwap(img, current.replace(/\.webp$/i, '.png'));
    });

    // En móvil usamos la portada vertical del Método MES.
    const courseImg = document.querySelector('.course-media img');
    const syncCourseAsset = () => {
      if(!courseImg) return;
    ...
    syncCourseAsset();
    window.addEventListener('resize', syncCourseAsset, {passive:true});
  }
