(() => {
  ...
  const formatDate = value => new Intl.DateTimeFormat('es-MX',{day:'2-digit',month:'short',year:'numeric',timeZone:'UTC'}).format(new Date(`${value}T12:00:00Z`));
  ...
  async function loadDashboard(user){
    ...
    if(wsError || !ws){ document.querySelector('[data-user-role]').textContent = 'No se encontró Academia Yamilet.'; return; }
    workspace = ws;
    ...
    document.querySelector('[data-user-role]').textContent = `${ws.name} · ${member?.role || profile?.role || 'alumno'}`;
    await Promise.all([loadCourses(), loadBookings(profile)]);
  }

  async function loadCourses(){
    const list = document.querySelector('[data-course-list]');
    const count = document.querySelector('[data-course-count]');
    const {data,error} = await sb.from('courses').select('id,title,subtitle,description,status,instructor_name,duration_label').eq('workspace_id',workspace.id).order('created_at',{ascending:true});
    if(error){ list.innerHTML = '<div class="empty">No fue posible cargar los cursos.</div>'; return; }
    count.textContent = data?.length || 0;
    if(!data?.length){ list.innerHTML = '<div class="empty">Los cursos aparecerán aquí conforme se publiquen.</div>'; return; }
    list.innerHTML = data.map(c => `<article class="course-card"><span class="tag">${c.status === 'published' ? 'Publicado' : 'En preparación'}</span><h3>${escapeHtml(c.title)}</h3><p>${escapeHtml(c.subtitle || c.description || 'Contenido de Academia Yamilet.')}</p><p><strong>Instructora:</strong> ${escapeHtml(c.instructor_name || 'Yamilet Pérez')}</p></article>`).join('');
  }

  async function loadBookings(profile){
    const list = document.querySelector('[data-booking-list]');
    const count = document.querySelector('[data-booking-count]');
    const isManager = (membership && ['owner','admin'].includes(membership.role)) || profile?.role === 'admin';
    if(!isManager){ list.innerHTML = '<div class="empty">Esta sección es visible para administradores.</div>'; count.textContent = '—'; return; }
    const {data,error} = await sb.from('free_class_bookings').select('id,requested_date,name,email,status').eq('workspace_id',workspace.id).order('requested_date',{ascending:true}).limit(100);
    if(error){ list.innerHTML = '<div class="empty">No fue posible cargar las reservaciones.</div>'; return; }
    count.textContent = (data || []).filter(x => x.status === 'pending').length;
    if(!data?.length){ list.innerHTML = '<div class="empty">Todavía no hay solicitudes de clase gratuita.</div>'; return; }
    list.innerHTML = data.map(b => `<article class="booking-row"><strong>${formatDate(b.requested_date)}</strong><span>${escapeHtml(b.name)}</span><span>${escapeHtml(b.email)}</span><select data-booking-status="${b.id}"><option value="pending" ${b.status==='pending'?'selected':''}>Pendiente</option><option value="confirmed" ${b.status==='confirmed'?'selected':''}>Confirmada</option><option value="completed" ${b.status==='completed'?'selected':''}>Completada</option><option value="cancelled" ${b.status==='cancelled'?'selected':''}>Cancelada</option></select></article>`).join('');
    list.querySelectorAll('[data-booking-status]').forEach(select => select.addEventListener('change', async () => {
      select.disabled = true;
      const {error:updateError} = await sb.from('free_class_bookings').update({status:select.value}).eq('id',select.dataset.bookingStatus);
      select.disabled = false;
      if(updateError) alert('No fue posible actualizar la reservación.'); else loadBookings(profile);
    }));
  }
  ...
  async function start(){
    try{
      ...
      loginForm?.addEventListener('submit', async e => {
        ...
        if(error){ setStatus('No se pudo iniciar sesión. Revisa tus datos o usa el enlace seguro por correo.'); return; }
      ...
      magicBtn?.addEventListener('click', async () => {
        ...
        if(!email){ setStatus('Escribe primero tu correo.'); return; }
        ...
        if(error){ setStatus('No fue posible enviar el enlace. Verifica que el correo esté registrado.'); return; }
      ...
      sb.auth.onAuthStateChange((_event,session) => { if(session?.user && dashboard.classList.contains('hidden')) loadDashboard(session.user); });
      const {data} = await sb.auth.getSession();
      if(data.session?.user) loadDashboard(data.session.user);
    }catch(err){ setStatus('No fue posible conectar con Academia Yamilet. Intenta nuevamente en unos minutos.'); }
  }
  start();
})();
