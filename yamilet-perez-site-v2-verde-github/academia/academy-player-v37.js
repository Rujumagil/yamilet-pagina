(() => {
  ...
  function outlineMarkup(courseId, lessonId) {
    ...
    return `<div class="mes-outline-head"><h3>Contenido del curso</h3><p>${esc(course.title)} · 4 semanas · 24 días</p></div>
      <div class="mes-course-progress"><div class="mes-course-progress-top"><span>Tu avance general</span><strong>${course.percent}%</strong></div><div class="mes-progress-track"><span style="width:${course.percent}%"></span></div></div>
  ...
  document.addEventListener('click', event => {
    ...
    if (event.target.closest('[data-open-course]')) setTimeout(schedule, 180);
    else if (row || event.target.closest('[data-toggle-complete]')) schedule(120);
  ...
  setInterval(() => {
    if (lessonView && !lessonView.classList.contains('hidden') && !lessonHost?.querySelector('.mes-player-shell')) schedule(0);
  }, 900);
