(() => {
  ...
  function setHidden(node, hidden) {
    if (!node) return;
    node.classList.toggle('v54-hidden', !!hidden);
  }
  ...
  function activeNav(view) {
    ...
    if (crumb) {
      if (view === 'lesson') crumb.textContent = document.querySelector('[data-lesson-detail] .lesson-title h2')?.textContent?.trim() || 'Método MES®';
      else crumb.textContent = labels[view] || 'Academia Yamilet';
    }
  }

  function updateHash(view, mode = 'push') {
    const hash = hashByView[view];
    if (!hash) return;
    const target = `#${hash}`;
    if (location.hash === target) return;
    history[mode === 'replace' ? 'replaceState' : 'pushState']({ academyView:view }, '', target);
  }

  function applyView(view, options = {}) {
    if (!dashboardOpen()) return;
    const { updateUrl = false, scroll = true } = options;
    managedNodes().forEach(node => setHidden(node, true));

    if (view === 'home') {
      [dashHead, stats, continuePanel].forEach(node => setHidden(node, false));
    } else if (view === 'courses') {
      setHidden(coursesPanel, false);
    } else if (view === 'course') {
      setHidden(courseView, false);
    } else if (view === 'lesson') {
      setHidden(lessonView, false);
    } else {
  ...
  function fixImages() {
    document.querySelectorAll('img.course-cover').forEach(img => {
      if (img.dataset.v54ImageGuard === '1') return;
      img.dataset.v54ImageGuard = '1';
      img.addEventListener('error', () => {
        if (img.dataset.v54Fallback === '1') return;
        img.dataset.v54Fallback = '1';
        img.src = '../assets/curso-metodo-mes.png?v=54';
      }, { once:true });
    });
    document.querySelectorAll('.side-brand img,.topbar .brand img,.login-logo').forEach(img => {
      if (img.dataset.v54LogoGuard === '1') return;
      img.dataset.v54LogoGuard = '1';
      img.addEventListener('error', () => { img.src = '../assets/logo-yamilet.png?v=54'; }, { once:true });
    });
  }
  ...
  function init() {
    ...
    queueRefresh();
    applyView(currentHashView(), { updateUrl:false, scroll:false });

    const mainObserver = new MutationObserver(queueRefresh);
    if (main) mainObserver.observe(main, { childList:true, subtree:true });

    [courseView, lessonView].forEach(node => node && new MutationObserver(() => {
      const learning = learningViewFromDom();
      if (learning) applyView(learning, { scroll:false });
  ...
  document.addEventListener('click', event => {
    ...
    if (shell) {
      ...
      if (hashByView[route]) setTimeout(() => applyView(route, { updateUrl:true }), 120);
      return;
    }
