(() => {
  ...
  const it=new Map(Object.entries({
    'Academia Yamilet':'Accademia Yamilet','Formación y Método MES®':'Formazione e Método MES®','Página principal':'Pagina principale','Entrar a mi academia':'Entra nella mia accademia','Cerrar':'Chiudi',
  ...
  function walk(root=document.body){
    ...
    try{
      ...
      document.querySelectorAll('a[href="../es/"],a[href="../it/"]').forEach(a=>a.setAttribute('href',locale()==='it'?'../it/':'../es/'));
      document.querySelectorAll('[data-public-lang]').forEach(b=>{const active=b.dataset.publicLang===locale();b.setAttribute('aria-pressed',active?'true':'false');b.style.background=active?'#123f35':'transparent';b.style.color=active?'#fff':'#123f35';});
