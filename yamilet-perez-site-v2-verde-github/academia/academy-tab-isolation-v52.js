(() => {
  ...
  function suppress(node, value) {
    if (!node) return;
    node.classList.toggle('ay52-suppressed', !!value);
  }
  ...
  function setNav(section) {
    ...
    document.querySelectorAll('[data-shell-route]').forEach(btn => {
      btn.classList.toggle('active', btn.dataset.shellRoute === activeRoute);
    });
    ...
    if (section === 'course') crumb.textContent = 'Método MES®';
      ...
      crumb.textContent = document.querySelector('[data-lesson-detail] .lesson-title h2')?.textContent?.trim() || 'Método MES®';
  ...
  function apply(section) {
    ...
    if (section === 'home') {
      suppress(coursesPanel, true);
      suppress(courseView, true);
      suppress(lessonView, true);
      suppress(reservations, true);
      pages.forEach(p => suppress(p, true));
    } else if (section === 'courses') {
      suppress(dashHead, true);
      suppress(stats, true);
      suppress(continuePanel, true);
      suppress(courseView, true);
      suppress(lessonView, true);
      suppress(reservations, true);
      pages.forEach(p => suppress(p, true));
  ...
  function routeFromHash() {
    ...
    const map = {
      inicio: 'home', cursos: 'courses', evaluaciones: 'evaluations', biblioteca: 'library',
      calendario: 'calendar', certificados: 'certificates', ayuda: 'help', perfil: 'profile', explorar: 'explore'
    };
  ...
  document.addEventListener('click', event => {
    ...
    if (shell && shell.dataset.shellRoute !== 'admin') {
      setTimeout(() => apply(shell.dataset.shellRoute), 40);
      return;
    }

    if (event.target.closest('[data-open-course]')) {
      setTimeout(() => apply('course'), 100);
      return;
    }

    if (event.target.closest('[data-open-lesson][data-course-id]')) {
      setTimeout(() => apply('lesson'), 100);
      return;
    }

    if (event.target.closest('[data-back-courses]')) {
      setTimeout(() => apply('courses'), 60);
      return;
    }

    if (event.target.closest('[data-back-course]')) {
      setTimeout(() => apply('course'), 60);
    }
  }, true);
