(() => {
  ...
  if (courseView) {
    new MutationObserver(() => sync(false)).observe(courseView, {
      attributes: true,
  ...
  if (lessonView) {
    new MutationObserver(() => sync(false)).observe(lessonView, {
      attributes: true,
      attributeFilter: ['class']
    });
  }
  ...
  window.addEventListener('hashchange', () => {
    if (!window.location.hash.includes('curso') && !window.location.hash.includes('leccion')) {
      setTimeout(() => sync(false), 30);
    }
  });
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => sync(false), { once: true });
  } else {
    sync(false);
  }

  window.ACADEMIA_YAMILET_DEDICATED_V51 = {
    release: RELEASE,
    sync,
    setMode: applyMode
  };
